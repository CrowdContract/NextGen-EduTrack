...
const handleUpload = () => {
  ...
  dispatch(
    uploadFiles({
      projectId: project._id, // ✅ guaranteed
      files: selectedFiles,
    })
  );
...
const handleDownloadFile = async (file) => {
  const res = await dispatch(
    downloadFile({
      ...
      fileId: file._id, // ✅ FIXED (was file_id ❌)
    })
  );
  ...
  if (res.meta.requestStatus === "fulfilled") {
    ...
    link.setAttribute(
      "download",
      file.originalName || file.name || "download" // ✅ better filename
    );
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div className="border-2 border-dashed border-slate-300 rounded-lg p-6 text-center hover:border-blue-400 transition-colors">
            <div className="mb-4">
              <span className="w-12 h-12 text-slate-400 mx-auto">📄</span>
            </div>
...
<div className="card">
  ...
  {(files || []).length === 0 ? (
    <div className="text-center py-4">
      <div className="w-16 h-16 text-slate-300 mx-auto mb-4">📁</div>
